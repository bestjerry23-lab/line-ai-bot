...
// API 路由（讓 orders.html 透過 Render 取得資料，避免 CORS）
  ...
  try {
      ...
      if (Date.now() - pending.timestamp < 5 * 60 * 1000) {
        ...
        if (parts.length >= 3) {
          ...
          const result = await callScript('addWaiting', {
            customer: parts[0], product: parts[1],
            price: parsePrice(parts[2]), cost: parsePrice(parts[3] || '0'),
            note: parts[4] || '', link: '',
    ...
    if (mergeMatch && pendingMerge[sourceId]) {
      ...
      const result = await callScript('mergeOrder', {
        orderId: mergeMatch[1], newProduct: pending.newProduct, newPrice: pending.newPrice,
      });
      delete pendingMerge[sourceId];
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 訂單已合併！\n━━━━━━━━━━━━━━\n📋 訂單編號：${result.orderId}\n📦 商品：${result.mergedProduct}\n💰 合計：${result.mergedPrice} 元` }] });
      }
      return;
    }
    ...
    if (userMessage === '不合併' && pendingMerge[sourceId]) {
      const pending = pendingMerge[sourceId];
      delete pendingMerge[sourceId];
      const result = await callScript('addOrder', { customer: pending.customer, product: pending.newProduct, price: pending.newPrice, forceNew: true });
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 已另開新訂單！\n━━━━━━━━━━━━━━\n📋 訂單編號：${result.orderId}\n👤 顧客：${pending.customer}\n📦 商品：${pending.newProduct}\n💰 金額：${pending.newPrice} 元\n⏳ 狀態：待付款` }] });
      return;
    }
    ...
    if (addProductMatch) {
      ...
      const result = await callScript('addProduct', { name: parts[0], price: parsePrice(parts[1]), cost: parsePrice(parts[2] || '0'), stock: Number(parts[3]) || 0, status: '現貨' });
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 商品已新增！\n🆔 編號：${result.id}\n📦 商品：${parts[0]}\n💰 售價：${parsePrice(parts[1])} 元\n💵 成本：${parsePrice(parts[2] || '0')} 元\n📦 庫存：${Number(parts[3]) || 0} 件` }] });
      return;
    }
    ...
    if (saleMatch) {
      ...
      const result = await callScript('addSale', { name: parts[0], id: parts[0], qty: Number(parts[1]) || 1 });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 銷售紀錄已新增！\n💰 銷售金額：${result.total} 元\n📈 獲利：${result.profit} 元\n📦 剩餘庫存：${result.newStock} 件` }] });
      }
      return;
    }
    ...
    if (addOrderMatch) {
      ...
      let cost = 0, isCOD = false;
      if (parts[3]) { if (parts[3] === '貨到付款') isCOD = true; else cost = parsePrice(parts[3]); }
      if (parts[4] === '貨到付款') isCOD = true;
      const result = await callScript('addOrder', { customer: parts[0], product: parts[1], price: parsePrice(parts[2]), cost, paymentType: isCOD ? '貨到付款' : '' });
      if (result.needMerge) {
        pendingMerge[sourceId] = { customer: parts[0], newProduct: parts[1], newPrice: parsePrice(parts[2]) };
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `⚠️ 偵測到同買家訂單！\n━━━━━━━━━━━━━━\n👤 顧客：${parts[0]}\n📋 現有訂單：${result.existingOrderId}\n📦 現有商品：${result.existingProduct}\n💰 現有金額：${result.existingPrice} 元\n━━━━━━━━━━━━━━\n🆕 新增商品：${parts[1]}\n💰 新增金額：${parsePrice(parts[2])} 元\n━━━━━━━━━━━━━━\n要合併到現有訂單嗎？\n✅ 回覆「合併：${result.existingOrderId}」\n❌ 回覆「不合併」另開新單` }] });
        return;
      }
      const statusText = isCOD ? '💵 貨到付款' : '⏳ 待付款';
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 訂單已建立！\n━━━━━━━━━━━━━━\n📋 訂單編號：${result.orderId}\n👤 顧客：${parts[0]}\n📦 商品：${parts[1]}\n💰 金額：${parsePrice(parts[2])} 元\n💵 成本：${cost || '未填'} 元\n${statusText}\n━━━━━━━━━━━━━━\n請將賣貨便連結傳給顧客 😊` }] });
      return;
    }
    ...
    if (payMatch) {
      ...
      const result = await callScript('updateOrder', { orderId: parts[0], status: '已付款', note: parts[1] || '' });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 付款已確認！\n━━━━━━━━━━━━━━\n📋 訂單：${parts[0]}\n💳 狀態：已付款\n📝 備註：${parts[1] || '無'}\n━━━━━━━━━━━━━━\n記得去跟韓國廠商下單！` }] });
      }
      return;
    }
    ...
    if (batchPurchaseMatch) {
      ...
      const result = await callScript('addPurchase', { orderId: parts[0], product: parts[1], cost: parsePrice(parts[2] || '0') });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 廠商下單完成！\n━━━━━━━━━━━━━━\n🛒 採購單號：${result.purchaseId}\n📋 關聯訂單：${result.orderId}\n📦 商品：${result.product}\n━━━━━━━━━━━━━━\n出貨時傳：部分出貨：${result.orderId}/${result.purchaseId}` }] });
      }
      return;
    }
    ...
    if (updateCostMatch) {
      ...
      const result = await callScript('updateCost', { purchaseId: parts[0], cost: parsePrice(parts[1]) });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 成本已更新！\n🛒 採購單：${parts[0]}\n💵 成本：${parsePrice(parts[1])} 元` }] });
      }
      return;
    }
    ...
    if (partialShipMatch) {
      ...
      const result = await callScript('partialShip', { orderId: parts[0], purchaseId: parts[1] });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        const allShippedMsg = result.allShipped ? '\n🎉 所有商品已全部出貨！\n可以傳「已完成：' + parts[0] + '」完成訂單' : '\n⏳ 還有其他商品待出貨';
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 部分出貨完成！\n━━━━━━━━━━━━━━\n📋 訂單：${parts[0]}\n🛒 採購單：${parts[1]}\n📦 狀態：已出貨${allShippedMsg}` }] });
      }
      return;
    }
    ...
    if (shipMatch) {
      const result = await callScript('updateOrder', { orderId: shipMatch[1], status: '已出貨' });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 已出貨！\n━━━━━━━━━━━━━━\n📋 訂單：${shipMatch[1]}\n📦 狀態：已出貨\n🚚 等待顧客收貨中...` }] });
      }
      return;
    }
    ...
    if (doneMatch) {
      const result = await callScript('updateOrder', { orderId: doneMatch[1], status: '已完成' });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        let stockMsg = '';
        if (result.stockUpdated && result.stockUpdated.length > 0) {
          stockMsg = '\n📦 庫存更新：\n' + result.stockUpdated.map(s => `• ${s.name}：剩餘 ${s.newStock} 件${s.newStock === 0 ? '（已售完）' : ''}`).join('\n');
        }
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `🎉 訂單完成！\n━━━━━━━━━━━━━━\n📋 訂單：${doneMatch[1]}\n✅ 狀態：已完成\n📈 獲利：${result.profit} 元${stockMsg}\n━━━━━━━━━━━━━━\n已自動寫入銷售紀錄！` }] });
      }
      return;
    }
    ...
    if (cancelMatch) {
      ...
      const result = await callScript('updateOrder', { orderId: parts[0], status: '已取消', note: parts[1] || '' });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        let refundMsg = result.needRefund ? '\n⚠️ 此訂單先前狀態為「' + result.previousStatus + '」\n請記得處理退款事宜！' : '';
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `🚫 訂單已取消！\n━━━━━━━━━━━━━━\n📋 訂單：${parts[0]}\n❌ 狀態：已取消\n📝 原因：${parts[1] || '無'}${refundMsg}` }] });
      }
      return;
    }
    ...
    if (editMatch) {
      ...
      const result = await callScript('editOrder', { orderId: parts[0], field: parts[1], value: parts[2] });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 訂單已修改！\n━━━━━━━━━━━━━━\n📋 訂單：${parts[0]}\n✏️ ${parts[1]}：${parts[2]}` }] });
      }
      return;
    }
    ...
    if (editWaitMatch) {
      ...
      if (parts.length < 3) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: '❌ 格式錯誤！\n正確格式：修改待訂：待訂編號/欄位/新內容\n\n可修改欄位：顧客、商品、金額、成本、備註、連結、付款' }] });
        return;
      }
      const result = await callScript('editWaiting', { waitId: parts[0], field: parts[1], value: parts[2] });
      if (result.error) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `❌ ${result.error}` }] });
      } else {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 待訂已修改！\n━━━━━━━━━━━━━━\n🆔 待訂：${parts[0]}\n✏️ ${parts[1]}：${parts[2]}` }] });
      }
      return;
    }
    ...
    if (userMessage === '待訂清單') {
      ...
      if (!result.list || result.list.length === 0) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: '目前沒有待訂記錄 😊' }] });
        return;
      }
      const text = `📝 待訂清單（${result.list.length} 筆）\n━━━━━━━━━━━━━━\n` +
        result.list.map(w => `🆔 ${w.id}\n👤 ${w.customer}\n📦 ${w.product}\n💰 ${w.price} 元｜💵 成本：${w.cost || '未填'} 元\n${w.paymentStatus === '已付款' ? '💳 已付款' : '⏳ 未付款'}\n📋 ${w.note || '無備註'}${w.link ? '\n🔗 ' + w.link : ''}${w.imageUrl ? '\n🖼️ 有圖片' : ''}\n📅 ${w.date}`).join('\n━━━━━━━━━━━━━━\n');
    ...
    if (convertMatch) {
      ...
      if (result.error) {
        ...
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `✅ 待訂已轉為正式訂單！\n━━━━━━━━━━━━━━\n📋 訂單編號：${result.orderId}\n👤 顧客：${result.customer}\n📦 商品：${result.product}\n💰 金額：${result.price} 元\n💵 成本：${result.cost || '未填'} 元\n${paymentMsg}\n━━━━━━━━━━━━━━\n${result.paymentStatus === '已付款' ? '已自動標記為已付款！' : '請將賣貨便連結傳給顧客 😊'}` }] });
      }
      return;
    }
    ...
    if (deleteWaitMatch) {
      ...
      if (result.error) {
        ...
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `🗑️ 待訂 ${deleteWaitMatch[1]} 已刪除！` }] });
      }
      return;
    }
    ...
    if (userMessage === '訂單管理' || userMessage === '開單系統' || userMessage === '訂單系統') {
      await client.replyMessage({
        replyToken: event.replyToken,
        messages: [{
          type: 'template', altText: '貝拉訂單管理系統',
          template: { type: 'buttons', title: '📋 訂單管理系統', text: '點下方按鈕開啟視覺化訂單管理',
    ...
    if (userMessage === '指令總覽' || userMessage === '查詢功能' || userMessage === '功能列表') {
      await client.replyMessage({
        replyToken: event.replyToken,
        messages: [{
          type: 'template', altText: '貝拉指令總覽',
          template: { type: 'buttons', title: '🤖 貝拉指令總覽', text: '點下方按鈕開啟完整指令選單',
    ...
    if (userMessage === '核對收貨' || userMessage === '開始核對' || userMessage === '叫出訂購單') {
      await client.replyMessage({
        replyToken: event.replyToken,
        messages: [{
          type: 'template', altText: '核對收貨系統',
          template: { type: 'buttons', title: '🛍️ 核對收貨系統', text: '點下方按鈕開啟核對介面',
    ...
    if (customerMatch) {
      ...
      if (!result.orders || result.orders.length === 0) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `找不到「${customerMatch[1]}」的訂單 😕` }] });
        return;
      }
    ...
    if (productMatch) {
      ...
      if (!result.orders || result.orders.length === 0) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `找不到「${productMatch[1]}」的相關訂單 😕` }] });
        return;
      }
      const statusEmoji = { '待付款':'⏳','貨到付款':'💵','已付款':'💳','已向廠商下單':'🛒','已出貨':'📦','已完成':'✅','已取消':'🚫' };
      ...
      text += result.orders.map(o => `${o.id} | 👤 ${o.customer}\n${statusEmoji[o.status]||'📌'} ${o.status} | 💰 ${o.price} 元`).join('\n━━━━━━━━━━━━━━\n');
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text }] });
      return;
    }
    ...
    if (orderIdMatch) {
      ...
      if (o.purchases && o.purchases.length > 0) {
        ...
        text += o.purchases.map(p => `${p.id} | ${p.product}\n💵 ${p.cost||'未填'} 元 | ${p.status}`).join('\n');
      }
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text }] });
      return;
    }
    ...
    if (userMessage === '訂單總覽') {
      ...
      if (!result.orders || result.orders.length === 0) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: '目前沒有任何訂單 😊' }] });
        return;
      }
      const active = result.orders.filter(o => o.status !== '已完成' && o.status !== '已取消');
      if (active.length === 0) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: '目前沒有進行中的訂單 🎉' }] });
        return;
      }
      const groups = {
      ...
      if (groups['待付款'].length > 0) { text += `⏳ 待付款（${groups['待付款'].length} 筆）\n` + groups['待付款'].map(o => `${o.id} ${o.customer}｜${o.product}｜${o.price}元`).join('\n') + '\n━━━━━━━━━━━━━━\n'; }
      if (groups['貨到付款'].length > 0) { text += `💵 貨到付款（${groups['貨到付款'].length} 筆）\n` + groups['貨到付款'].map(o => `${o.id} ${o.customer}｜${o.product}｜${o.price}元`).join('\n') + '\n━━━━━━━━━━━━━━\n'; }
      if (groups['待採購'].length > 0) { text += `💳 待採購（${groups['待採購'].length} 筆）\n` + groups['待採購'].map(o => `${o.id} ${o.customer}｜${o.product}｜${o.price}元`).join('\n') + '\n━━━━━━━━━━━━━━\n'; }
      if (groups['待出貨'].length > 0) { text += `🛒 待出貨（${groups['待出貨'].length} 筆）\n` + groups['待出貨'].map(o => `${o.id} ${o.customer}｜${o.product}｜${o.price}元`).join('\n') + '\n━━━━━━━━━━━━━━\n'; }
      if (groups['已出貨'].length > 0) { text += `📦 已出貨（${groups['已出貨'].length} 筆）\n` + groups['已出貨'].map(o => `${o.id} ${o.customer}｜${o.product}｜${o.price}元`).join('\n'); }
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text }] });
      return;
    }
    ...
    if (statusMap[userMessage]) {
      ...
      if (!result.orders || result.orders.length === 0) {
        await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: `目前沒有「${userMessage}」的訂單 😊` }] });
        return;
      }
      const text = `📋 ${userMessage}訂單（${result.orders.length} 筆）\n━━━━━━━━━━━━━━\n` +
        result.orders.map(o => `🆔 ${o.id} | 👤 ${o.customer}\n📦 ${o.product} | 💰 ${o.price} 元`).join('\n━━━━━━━━━━━━━━\n');
      await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text }] });
      return;
    }
    ...
    await client.replyMessage({ replyToken: event.replyToken, messages: [{ type: 'text', text: aiReply }] });
